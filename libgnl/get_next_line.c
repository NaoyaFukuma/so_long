...
char	*ft_split_and_store_text(char *str, size_t strlen, t_fd_info *lst_p)
{
	char	*newline;
	char	*tmp_str;
	size_t	i;
...
t_fd_info	*ft_newnode(int fd, t_fd_info *previous)
{
	t_fd_info	*newnode;

	newnode = malloc(sizeof(t_fd_info));
	if (newnode == NULL)
		return (NULL);
	newnode->fd = fd;
...
t_fd_info	*ft_set_pointer(t_fd_info **static_p, t_fd_info **lst_p, int fd)
{
	if (*static_p == NULL)
